    ...
    inline void decode(Graph* pcg, Instance& inst, bool nerOnly, const vector<CAction>* goldAC = NULL) {
        ...
        while (true) {
            ...
            if (pcg->train && pGoldGenerator == NULL) {
                std::cout << "gold state is droppped, bug exists" << std::endl;
            }
            ...
            for (int idx = 0; idx < lastStates.size(); idx++) {
                ...
                for (int idy = 0; idy < actions[idx].size(); ++idy) {
                    ...
                    if (pcg->train) {
                        ...
                        if (actions[idx][idy] != answer) errorAnswer = true;
                        if (pGenerator->_atomFeat.bRel) {
                            if (pGenerator->_atomFeat.rel_i != pGoldGenerator->_atomFeat.rel_i || pGenerator->_atomFeat.rel_j != pGoldGenerator->_atomFeat.rel_j) {
                                errorAnswer = true;
                            }
                        }
                        if (errorAnswer)pGenerator->_nextscores.outputs[idy].val[0] += pOpts->delta;
                    }
