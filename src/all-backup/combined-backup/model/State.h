...
class CStateItem {
  ...
  public:
    bool allow_ner() const {
        if (_current_i <= 0 && _current_j + 1 < _word_size) {
            return true;
        }
        return false;
    }
  ...
  public:
    void move(CStateItem *next, const CAction &ac) {
        ...
        if (next->_current_j == _word_size - 1 && next->_current_i <= 0) {
            next->_bEnd = true;
        }
    }
    ...
    void getResults(CResult &result, HyperParams &opts) const {
        ...
        int count = 0;
        while (!curr->_bStart) {
            if (curr->_label != _labels[curr->_current_i][curr->_current_j]) {
                std::cout << "result error, debug" << std::endl;
            }
            if (curr->_current_i == curr->_current_j) {
                result.relations[curr->_current_i][curr->_current_i] = opts.ner_labels.from_id(curr->_label);
            } else {
                short labelId = curr->_label;
                if (labelId > 0) {
                    result.relations[curr->_current_i][curr->_current_j] = opts.rel_labels.from_id(labelId);
                    result.directions[curr->_current_i][curr->_current_j] = 1;
                } else if (labelId < 0) {
                    result.relations[curr->_current_i][curr->_current_j] = opts.rel_labels.from_id(-labelId);
                    result.directions[curr->_current_i][curr->_current_j] = 2;
                } else {
                    result.relations[curr->_current_i][curr->_current_j] = opts.rel_labels.from_id(0);
                    result.directions[curr->_current_i][curr->_current_j] = 0;
                }
            }
    ...
    void getGoldAction(HyperParams &opts, const CResult &result, CAction &ac) const {
        if (allow_ner()) {
            ac.set(CAction::NER, opts.ner_labels.from_string(result.relations[_current_j + 1][_current_j + 1]));
            return;
        }

        if (allow_rel()) {
            int rel_labelId = opts.rel_labels.from_string(result.relations[_current_i - 1][_current_j]);
            if (rel_labelId == 0 || result.directions[_current_i - 1][_current_j] == 1) {
            } else if (result.directions[_current_i - 1][_current_j] == 2) {
    ...
    void getCandidateActions(vector<CAction> &actions, HyperParams* opts, ModelParams* params) const {
        ...
        if (allow_ner()) {
            ...
            int modvalue = label_j % 4;
            if (_current_j < 0 || modvalue == 0 || modvalue == 3) { //o, e-xx, s-xx
                ...
                ac.set(CAction::NER, label_j + 1);  //m-xx
                if (params->embeded_actions.from_string(ac.str(opts)) >= 0)actions.push_back(ac);
                ac.set(CAction::NER, label_j + 2);  //e-xx
                if (params->embeded_actions.from_string(ac.str(opts)) >= 0)actions.push_back(ac);
            } else { // m-xx
                ac.set(CAction::NER, label_j);  //m-xx
                if (params->embeded_actions.from_string(ac.str(opts)) >= 0)actions.push_back(ac);
                ac.set(CAction::NER, label_j + 1);  //e-xx
                if (params->embeded_actions.from_string(ac.str(opts)) >= 0)actions.push_back(ac);
            }
        ...
        if (allow_rel()) {
            short label_i = getNERId(_current_i - 1);
            short label_j = getNERId(_current_j);
  ...
  public:
    ...
    inline void prepare(HyperParams* hyper_params, ModelParams* model_params, GlobalNodes* global_nodes) {
        _atomFeat.clear();
        if (allow_ner()) {
            _atomFeat.ner_next_position = _current_j + 1;
            ...
            _atomFeat.rel_i = _current_i - 1;
            _atomFeat.rel_j = _current_j;
