...
struct ActionedNodes {
  ...
  public:
    inline void initial(ModelParams &params, HyperParams &hyparams) {
        ...
        last_ner_input.init(hyparams.ner_dim, hyparams.dropProb);
        ner_lstm.init(&(params.ner_lstm), hyparams.dropProb); //already allocated here
        ...
        left_ner_entityi.init(hyparams.ner_lstm_dim, -1);
        left_ner_entityj.init(hyparams.ner_lstm_dim, -1);

        rel_state_represent.init(hyparams.rel_state_concat_dim, -1);
        rel_state_hidden.setParam(&(params.rel_state_hidden));
        rel_state_hidden.init(hyparams.state_hidden_dim, -1);
        ...
        bucket_ner.init(hyparams.ner_lstm_dim, -1);
        bucket_word.init(hyparams.word_lstm_dim, -1);
        bucket_state.init(hyparams.state_hidden_dim, -1);
    }
