...
class CStateItem {
  ...
  public:
    ...
    void getCandidateActions(vector<CAction> &actions, HyperParams* opts, ModelParams* params, bool bGreedyTrain) const {
        actions.clear();
        CAction ac;
        ...
        if (allow_rel()) {
            ...
            if (label_i > 0 && label_j > 0
                ...
                if (rel_id != -1) {
                    ...
                    if (params->embeded_rel_actions.from_string(ac.str(opts)) >= 0)actions.push_back(ac);
                }
  ...
  public:
    ...
    void getOrcaleAction(HyperParams &opts, const CResult &result, CAction &ac) const {
        ...
        if (allow_ner()) {
            ...
            int modvalue = _lastAction._label % 4;
            if (modvalue == 0 || modvalue == 3) { //o, e-xx, s-xx
                if (goldlabel % 4 == 0 || goldlabel % 4 == 1) {
                    find = true;
                } else {
                    bestlabel = ((int)(goldlabel / 4)) * 4;
                }
            } else if (modvalue == 1) { //b-xx
                if (goldlabel == _lastAction._label + 1 || goldlabel == _lastAction._label + 2) {
                    find = true;
                } else {
                    bestlabel = _lastAction._label + 2;
                }
            } else { // m-xx
                if (goldlabel == _lastAction._label || goldlabel == _lastAction._label + 1) {
                    find = true;
                } else {
                    bestlabel = _lastAction._label + 1;
                }
            }

            if (find) {
                ac.set(CAction::NER, goldlabel);
            } else {
        ...
        if (allow_rel()) {
            ...
            if (gold_rel_labelId == 0 || result.directions[next_i][next_j] == 1) {
            } else if (result.directions[next_i][next_j] == 2) {
            ...
            if (label_i > 0 && label_j > 0
                ...
                if (ij_rel_labelid <= 0 && ji_rel_labelid <= 0) {
                    std::cout << "next_i or next_j error" << std::endl;
                    return;
                }

                if (next_head_index == _head_size - 1 && _headed_entities[next_entity_index] == 0) {
                    if (ij_rel_labelid > 0) {
                        ac.set(CAction::REL, ij_rel_labelid);
                    } else {
                        ac.set(CAction::REL, -ji_rel_labelid);
                    }
                } else {
                    if (gold_rel_labelId == 0) {
                        ac.set(CAction::REL, 0);
                    } else if (gold_rel_labelId > 0) {
                        ac.set(CAction::REL, ij_rel_labelid);
                        if (ij_rel_labelid != gold_rel_labelId) {
                            std::cout << "strange orcale action, rel label not match" << std::endl;
                        }
                    } else {
                        ac.set(CAction::REL, -ji_rel_labelid);
                        if (-ji_rel_labelid != gold_rel_labelId) {
                            std::cout << "strange orcale action, rel label not match" << std::endl;
                        }
                    }
                }
...
bool state_compare(const CScoredState &o1, const CScoredState &o2) {
