  ...
  public:
    ...
    inline void prepare(HyperParams* hyper_params, ModelParams* model_params, GlobalNodes* global_nodes) {
        ...
        if (allow_ner()) {
            ...
            _atomFeat.ner1_label = nullkey;
            _atomFeat.ner2_label = nullkey;

            int modvalue_i = label_i % 4;
            int modvalue_j = label_j % 4;

            if (label_i > 0 && label_j > 0 && _atomFeat.rel_j_start > _atomFeat.rel_i_start
                    && (modvalue_i == 0 || modvalue_i == 3)
                    && (modvalue_j == 0 || modvalue_j == 3)) {
                _atomFeat.rel_must_o = 0;
                _atomFeat.ner1_label = hyper_params->ner_labels.from_id(label_i);
                _atomFeat.ner2_label = hyper_params->ner_labels.from_id(label_j);
            } else {
