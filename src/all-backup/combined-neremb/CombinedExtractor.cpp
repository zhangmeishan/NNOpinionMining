...
int Extractor::createAlphabet(vector<Instance> &vecInsts) {
	...
	m_driver._modelparams.embeded_ners.clear();
	m_driver._modelparams.embeded_ners.from_string("o");	
    ...
    for (siter = nerlabels.begin(); siter != nerlabels.end(); siter++) {
		...
		m_driver._modelparams.embeded_ners.from_string("b-" + *siter);
		m_driver._modelparams.embeded_ners.from_string("m-" + *siter);
		m_driver._modelparams.embeded_ners.from_string("e-" + *siter);
		m_driver._modelparams.embeded_ners.from_string("s-" + *siter);
    }
    m_driver._hyperparams.ner_labels.set_fixed_flag(true);
    m_driver._hyperparams.ner_noprefix_num = nerlabels.size();

	m_driver._modelparams.embeded_ners.from_string(nullkey);
	m_driver._modelparams.embeded_ners.set_fixed_flag(true);
...
void Extractor::train(const string &trainFile, const string &devFile, const string &testFile, const string &modelFile,
    ...
    bool succeed = m_driver._modelparams.word_ext_table.initial(&m_driver._modelparams.embeded_ext_words, m_options.wordEmbFile, false, m_options.wordEmbNormalize);
	if (!succeed) {
		getchar();
		return;
	}
    m_options.wordExtEmbSize = m_driver._modelparams.word_ext_table.nDim;
	m_options.wordEmbSize = m_options.wordExtEmbSize;
