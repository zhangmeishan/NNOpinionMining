...
class Driver {
    ...
    dtype train(std::vector<Instance > &sentences, const vector<vector<CAction> > &goldACs, bool nerOnly) {
        ...
        if(_useBeam) {
            ...
            for (int idx = 0; idx < num; idx++) {
                ...
                //compute reward
                //reward_computation_oracle(_beam_builders[idx], (sentences[idx]), upper_step - 1);
  ...
  private:
    ...
    dtype loss_google_reinforce(BeamGraphBuilder& builder, int upper_step, int num) {
        int maxstep = builder.outputs.size();
        if (maxstep == 0) return 1.0;
        if (upper_step > 0 && maxstep > upper_step) maxstep = upper_step;
        PNode pBestNode = NULL;
    ...
    void reward_computation_oracle(BeamGraphBuilder& builder, Instance& inst, int last_step) {
