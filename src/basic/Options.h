    ...
    void setOptions(const vector<string> &vecOption) {
        ...
        for (; i < vecOption.size(); ++i) {
            ...
            if (pr.first == "wordEmbNormalize")
                wordEmbNormalize = atof(pr.second.c_str());
            ...
            if (pr.first == "outBest")
                outBest = pr.second;
            if (pr.first == "unk_strategy")
                unk_strategy = atoi(pr.second.c_str());
            if (pr.first == "lstm_layer")
